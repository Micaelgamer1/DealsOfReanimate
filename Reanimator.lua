...
local UhhhhhhVersion = "REX VVIP | ADVANCED ANTI-HATDROP"

-- =========================================================================
-- FIX #2: cloneref (and friends) must be defined BEFORE any use
-- =========================================================================
...
-- =========================================================================
-- FIX #3: removed bogus pcall/xpcall overrides (they were pointless
-- recursion traps with no benefit)
-- =========================================================================
...
-- =========================================================================
-- FIX #1: removed the duplicated `local conn conn` (syntax error)
-- =========================================================================
...
-- =========================================================================
-- FIX #7: PlayMusic now caps retry attempts to avoid infinite recursion
-- when every music file fails to load (common on mobile executors).
-- =========================================================================
MusicPlayer._FailStreak = 0
MusicPlayer.PlayMusic = function(i)
	...
	if #soundid == 0 then
		MusicPlayer.Switching = false
		MusicPlayer._FailStreak = (MusicPlayer._FailStreak or 0) + 1
		if MusicPlayer._FailStreak >= #MusicPlayer.Database then
			-- Gave up: all tracks are unreadable on this executor.
			MusicPlayer._FailStreak = 0
			return
		end
...
local CracktroFrameText = "REX VVIP | UHHHH REANIMATE"
local UIMainWindow, WindowContent do
	...
	TopBarText.Text = "REX VVIP | UHHHH REANIMATE | v" .. UhhhhhhVersion
...
function UI.CreateDropdown(parent, text, array, value)
	...
	-- =========================================================================
	-- FIX #10: dropdown no longer leaks its scrollframe listener
	-- =========================================================================
	local scrollframe = Container:FindFirstAncestorWhichIsA("ScrollingFrame")
	if scrollframe then
		local scrollconn = scrollframe:GetPropertyChangedSignal("CanvasPosition"):Connect(function()
			opened = false
		end)
		Util.LinkDestroyI2C(Droplist, scrollconn)
	end
	local opentween = 0
	AddToRenderStep(function(t, dt)
		if opened then
...
-- =========================================================================
-- FIX #11: notify user when the intro is still loading instead of silently
-- eating their clicks.
-- =========================================================================
local _cracktroWaitNotified = false
CracktroFrame.InputEnded:Connect(function(input)
	if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
		if _totalrendertime < 1 or not IsUhhhhhhFullyLoaded then
			if not _cracktroWaitNotified then
				_cracktroWaitNotified = true
				Util.UINotify("aguarde, ainda carregando...")
			end
			return
		end
			...
			if Reanimate.Character then
				...
				if Humanoid and RootPart and Camera.CameraSubject == Humanoid then
					if self.Scriptable then
						...
						-- =====================================================
						-- FIX #8: removed ghost `suppliedLookVector` reference
						-- =====================================================
...
local HatReanimator = {}
local REX_VVIP_CACHE = setmetatable({}, {__mode = "k"})
local REX_VVIP_ACTIVE = false
local REX_VVIP_LAST_TCF = setmetatable({}, {__mode = "k"})
local REX_VVIP_LAST_SEEN = setmetatable({}, {__mode = "k"})

local function REX_VVIP_IsFiniteNumber(n)
    return type(n) == "number" and n == n and n > -1e10 and n < 1e10
end

local function REX_VVIP_IsSafeCFrame(cf)
    if typeof(cf) ~= "CFrame" then
        return false
    end
    local p = cf.Position
    return REX_VVIP_IsFiniteNumber(p.X)
        and REX_VVIP_IsFiniteNumber(p.Y)
        and REX_VVIP_IsFiniteNumber(p.Z)
        and p.Magnitude < 1000000
end

local function REX_VVIP_CacheHat(hat)
    if not hat or not hat:IsA("Accessory") then
        return
    end
    ...
    local c = REX_VVIP_CACHE[hat]
    if not c or c.Handle ~= handle then
        c = {
            Hat = hat,
            Handle = handle,
            Transparency = handle.Transparency,
            OriginalLTM = handle.LocalTransparencyModifier,
            CanCollide = handle.CanCollide,
            Massless = handle.Massless,
            LastGoodCFrame = handle.CFrame,
            LastGoodTarget = nil,
            LastGoodVelocity = Vector3.zero,
        }
        REX_VVIP_CACHE[hat] = c
    end

    if handle.Parent then
        c.LastGoodCFrame = handle.CFrame
        REX_VVIP_LAST_SEEN[hat] = os.clock()
    end
end

local function REX_VVIP_ProtectVisible(hat)
    local c = REX_VVIP_CACHE[hat]
    if not c then
        REX_VVIP_CacheHat(hat)
        c = REX_VVIP_CACHE[hat]
    end
    if not c then
        return
    end
    ...
    pcall(function()
        h.Transparency = c.Transparency
        h.LocalTransparencyModifier = 0
        h.Massless = c.Massless
...
local function REX_VVIP_SetEquipped(hat)
    if hat and hat.Parent then
        pcall(SetAccoutrementState, hat, BackendAccoutrementState.Equipped)
    end
end

local function REX_VVIP_CacheAll(hats)
    for _, hat in hats do
        REX_VVIP_CacheHat(hat)
        REX_VVIP_ProtectVisible(hat)
    end
end

HatReanimator.Name = "REX"
SaveData.Reanimator.HatsCollide = not not SaveData.Reanimator.HatsCollide
if SaveData.Reanimator.HatsCollideMethod == nil
    or SaveData.Reanimator.HatsCollideMethod == 6
    or SaveData.Reanimator.HatsCollideMethod == 8 then
    SaveData.Reanimator.HatsCollideMethod = 7
end
...
SaveData.Reanimator.HatsFling = false
SaveData.Reanimator.HatsSpin = false
SaveData.Reanimator.HatsFlingMethod = -1
...
HatReanimator.Permadeath = true

-- =========================================================================
-- FIX #9: track whether permadeath is actually supported by this executor
-- so the UI doesn't lie to the user.
-- =========================================================================
HatReanimator.PermadeathSupported = not not replicatesignal
...
function HatReanimator.Config(parent)
	...
	-- =========================================================================
	-- FIX #9 (part 2): UI now reflects real Permadeath support
	-- =========================================================================
	local permadeathLabel = "Permadeath"
	if not HatReanimator.PermadeathSupported then
		permadeathLabel = "Permadeath (indisponível: sem replicatesignal)"
	end
	UI.CreateSwitch(parent, permadeathLabel,
		HatReanimator.PermadeathSupported and HatReanimator.Permadeath
	).Changed:Connect(function(val)
		if not HatReanimator.PermadeathSupported then
	...
	UI.CreateText(parent, "<font color=\"#00DDFF\">REX VVIP: 1.2MB+ ULTRA ANTI-HATDROP / VISIBLE HATS++ / NO CHAR FLING</font>", 12, Enum.TextXAlignment.Center)
	UI.CreateText(parent, "Method 8 = REX VVIP • character/root fling blocked • hat state guarded", 10, Enum.TextXAlignment.Center)
	...
	UI.CreateDropdown(parent, "Torso Offset", {
		...
		"8 - REX VVIP (STEVE METHOD V3) - 1.2MB+ ULTRA ANTI-HATDROP / VISIBLE HATS++ / NO CHAR FLING",
		"9 - experimental / disabled for REX VVIP",
...
function HatReanimator.Start()
	-- =========================================================================
	-- FIX #6 (partial): reset the active flag at the start of every session
	-- =========================================================================
	REX_VVIP_ACTIVE = false
	...
	-- =========================================================================
	-- FIX #4: REXVVIP_DirectWeldTarget moved OUT of GetHatMappedCFrame so the
	-- main alignment loop can actually call it (it was scoped-wrong before).
	-- =========================================================================
	local function REXVVIP_DirectWeldTarget(hat)
		local ReanimCharacter = Reanimate.Character
		local sourceCharacter = Player.Character
		if not ReanimCharacter or not sourceCharacter then
			return
		end
		...
		for _,att in handle:GetChildren() do
			if att:IsA("Attachment") then
				local targetAtt = ReanimCharacter:FindFirstChild(att.Name, true)
				if targetAtt and targetAtt:IsA("Attachment") and targetAtt.Parent:IsA("BasePart") then
					return targetAtt.WorldCFrame * att.CFrame:Inverse(),
						targetAtt.Parent.Velocity
				end
			end
		end
		...
		local sourceLimb = nil
		if weld.Part0 and weld.Part0 ~= handle then
			sourceLimb = weld.Part0
		elseif weld.Part1 and weld.Part1 ~= handle then
			sourceLimb = weld.Part1
		end

		if not sourceLimb or not sourceLimb:IsDescendantOf(sourceCharacter) then
			return
		end

		local limb = ReanimCharacter:FindFirstChild(sourceLimb.Name, true)
		if limb and limb:IsA("BasePart") then
			return limb.CFrame * weld.C1 * weld.C0:Inverse(), limb.Velocity
		end
	end
	...
	HatCollideMethods[7] = {
				...
				for _,v in character:GetDescendants() do
					if v:IsA("Motor6D") then
						...
						elseif v.Name:find("Shoulder") or v.Name:find("Hip") then
							Util.SetMotor6DOffset(v, CFrame.new(v.C0.X - v.C1.X, 0, -0.5))
						else
							Util.SetMotor6DTransform(v, CFrame.identity)
						end
					end
		...
		State1 = function(character, Humanoid, hats)
			REX_VVIP_ACTIVE = true
			for _,hat in hats do
				REX_VVIP_CacheHat(hat)
				REX_VVIP_ProtectVisible(hat)
			end
			HatReanimator.Status.HatCollide = "REX VVIP: hats preserved"
		end,

		State2 = function(character, hats)
			REX_VVIP_ACTIVE = true
			HatReanimator.Status.HatCollide = "REX VVIP: final attachment lock"
			...
			-- Só espera Torso/Head sumirem se o personagem realmente morreu.
			-- Sem replicatesignal (ou permadeath off), o personagem fica vivo
			-- e nunca é destruído, o que travaria esse State2 pra sempre.
			if dead then
				if torso then
					task.wait(math.max(0,
						calculatepartdestroytime(2, 26, workspace.Gravity) - 0.1
					))
				end
			...
			for _ = 1, 20 do
				...
				HatReanimator.Status.HatCollide =
					("REX VVIP: %d hats locked"):format(alive)
				task.wait(0.08)
			end

			-- =============================================================
			-- FIX #6: release the watchdog when the session ends
			-- =============================================================
			REX_VVIP_ACTIVE = false
			return _counthats(hats)
		end,
	...
	-- =========================================================================
	-- FIX #5: watchdog now uses the correct REX_VVIP_* names (was REZ_V11_*)
	-- =========================================================================
	RunService.Heartbeat:Connect(function()
		if not REX_VVIP_ACTIVE then
			return
		end
		...
		for _,obj in character:GetChildren() do
			if obj:IsA("Accessory") then
				REX_VVIP_CacheHat(obj)
				REX_VVIP_ProtectVisible(obj)
			end
		end
	end)
	...
	local function OnCharacter(character)
		...
		if perma then
			...
			-- REX VVIP sem permadeath: NÃO mata o personagem.
			-- Deixa ele congelado em Swimming pra não respawnar em loop.
			Humanoid.EvaluateStateMachine = false
			Humanoid:ChangeState(Enum.HumanoidStateType.Swimming)
		end
		readystate = 3
			...
			if RCRootPart then
				...
				for _,hat in CharHats do
					...
					if handle and handle:IsA("BasePart") then
						...
						local mapped = ref and GetHatMappedOverride(ref.Map) or nil
						local tcf, tvel = GetHatMappedCFrame(mapped)

						if not REX_VVIP_IsSafeCFrame(tcf) then
							tcf, tvel = REXVVIP_DirectWeldTarget(hat)
						end
						...
						if not REX_VVIP_IsSafeCFrame(tcf) then
							if HatReanimator.RebuildRequired then
								...
								RefHatToHatRefs(hat)
								ref = Hat2HatRefs[hat]
								mapped = ref and GetHatMappedOverride(ref.Map) or nil
								tcf, tvel = GetHatMappedCFrame(mapped)
							end
						end

						local cache = REX_VVIP_CACHE[hat]
						if not REX_VVIP_IsSafeCFrame(tcf) then
							tcf = cache and cache.LastGoodTarget or handle.CFrame
							tvel = Vector3.zero
						end

						if cache and REX_VVIP_IsSafeCFrame(cache.LastGoodTarget)
							and REX_VVIP_IsSafeCFrame(tcf) then
							local jump = (tcf.Position - cache.LastGoodTarget.Position).Magnitude
							if jump > 45 then
								tcf = cache.LastGoodTarget
								tvel = Vector3.zero
							end
						end

						if cache then
							cache.LastGoodTarget = tcf
							cache.LastGoodCFrame = handle.CFrame
							cache.LastGoodVelocity =
								(typeof(tvel) == "Vector3" and tvel.Magnitude < 100000)
								and tvel or Vector3.zero
						end

						if typeof(tvel) ~= "Vector3" or tvel.Magnitude > 100000 then
							tvel = Vector3.zero
						end
						...
						pcall(sethiddenproperty,
							handle,
							"PhysicsRepRootPart",
							mapped and mapped.RepRootPart or nil
						)
					end
				end
...
-- REX VVIP FINAL BUILD: existing-hat preservation + visibility guard + no character fling.